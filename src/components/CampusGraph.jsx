...
function CampusGraph() {
      ...
      sampleData.nodes.forEach((node) => {
        ...
        el.addEventListener("click", () => {
          ...
          new maplibregl.Popup({ offset: 25 })
            ...
            .setHTML(
              `<div style="background:#1e1e1e;color:white;padding:10px;border-radius:10px;border:2px solid ${cfg.color};"><h4 style="margin:0">${node.name}</h4></div>`,
            )
            .addTo(map);
        });
  ...
  useEffect(() => {
    ...
    if (routeMode === "navigate") {
      ...
      setRouteInfo({
        distance: Math.round(remainingDistanceMeters),
        time: walkTimeMins < 1 ? "< 1" : walkTimeMins,
      });
    ...
    try {
      ...
      edges.forEach((edge) => {
        ...
        if (coordsArray.length === 0)
          coordsArray.push([source.longitude, source.latitude]);
    ...
    <div style={{ position: "relative" }}>
      ...
      {routeInfo && routeMode && (
        ...
        >
          ...
          <div
            style={{ height: "30px", width: "1px", background: "#555" }}
          ></div>
