...
export default function CampusGraph() {
          ...
          geometry: {
            ...
            coordinates: [
              [
                [lng, lat],
                [
                  lng + R * Math.sin(toR(h - sp)),
                  lat + R * Math.cos(toR(h - sp)),
                ],
                [
                  lng + R * Math.sin(toR(h + sp)),
                  lat + R * Math.cos(toR(h + sp)),
  ...
  useEffect(() => {
    ...
    const latChanged =
      Math.abs((prevLatRef.current ?? 0) - coords.lat) > 0.000001;
    const lngChanged =
      Math.abs((prevLngRef.current ?? 0) - coords.lng) > 0.000001;
    const hdgChanged =
      hdg == null
        ? prevHeadingRef.current != null
        : Math.abs(
            ((hdg - (prevHeadingRef.current ?? hdg) + 540) % 360) - 180,
          ) > 2;

    if (latChanged || lngChanged || hdgChanged) {
      map
        .getSource("heading-src")
        ?.setData(buildHeadingCone(coords.lat, coords.lng, hdg));
  ...
  const triggerReroute = useCallback(async (currentCoords) => {
    ...
    try {
      ...
      if (arr.length === 0) {
        try {
          arr = await fetchOSRMRoute(nearestNode, dest);
        } catch (e) {
  ...
  useEffect(() => {
    ...
    if (dirs.length > 0) {
      ...
      if (nextTurnStep && rc[nextTurnStep.coordIndex]) {
        ...
        for (
          let i = closestIdx;
          i < nextTurnStep.coordIndex && i < rc.length - 1;
          i++
        ) {
          d += getDistanceInMeters(
            rc[i][1],
            rc[i][0],
            rc[i + 1][1],
            rc[i + 1][0],
    ...
    try {
      ...
      if (arr.length === 0) {
        try {
          ...
          console.warn("OSRM fallback failed", e);
          arr = [
            [activeSource.longitude, activeSource.latitude],
  ...
  const StepsPanel = () => (
    ...
    >
      ...
      <div
        ref={stepsListRef}
        style={{ overflowY: "auto", padding: "8px 12px 4px", flex: 1 }}
      >
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 12, color: "#fbbc04" }}>
              Walk to <strong>{selectedSource?.name}</strong> to begin
              navigation
            </div>
          </div>
        </div>
            ...
            >
              {routeInfo && (
                ...
                >
                  ...
                  <span
                    style={{ fontSize: 16, fontWeight: 600, color: "#999" }}
                  >
                    {routeInfo.distance} m
                  </span>
                  {selectedDest && (
