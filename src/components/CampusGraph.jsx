...
// Off-route threshold: 5 m — if the user strays further than this from
// the nearest route point a reroute is triggered immediately.
const OFF_ROUTE_THRESHOLD_M = 5;
...
export default function CampusGraph() {
    ...
    data.nodes
      ...
      .forEach((node) => {
        ...
        Object.assign(el.style, {
          ...
          alignItems: "center", cursor: "pointer", gap: "2px",
        });
  ...
  useEffect(() => {
    ...
    if (!userMarkerRef.current) {
      ...
      dot.style.cssText =
        ...
        "border-radius:50%;box-shadow:0 0 12px rgba(66,133,244,0.9);";
  ...
  const triggerReroute = useCallback(async (currentCoords) => {
    ...
    // Find nearest graph node to the user
    ...
    try {
      ...
      // Prepend user's live GPS position as the route start
  ...
  useEffect(() => {
    ...
    // ── Active step + distance to next turn ───────────────────────────────
  ...
  const clearRoute = () => {
    ...
    setTravelledIdx(0);
    setRouteReady(false); setNavigating(false);
    setDirections([]); directionsRef.current = [];
    setStepIdx(0); stepIdxRef.current = 0;
    setRouteInfo(null); setArrived(false);
    setIsRerouting(false);
    isReroutingRef.current = false;
    lastRerouteTimeRef.current = 0;
    autoFollowRef.current = false;
  ...
  const StepsPanel = () => (
    ...
    >
      <div style={{
        ...
        padding: "14px 16px 10px", borderBottom: "1px solid #1e1e1e", flexShrink: 0,
      }}>
        <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, overflow: "hidden", background: "#000" }}>
      ...
      {isRerouting && navigating && (
        <div style={{
          position: "absolute", top: 16, left: 12, right: 12, zIndex: 26, marginTop: 50,
          background: "rgba(10,10,10,0.97)", border: "2.5px solid #fbbc04",
          ...
          <div style={{
            ...
            background: "#fbbc0418", border: "2px solid #fbbc04",
            display: "flex", alignItems: "center", justifyContent: "center",
      ...
      {arrived && (
        <div style={{
          ...
          <button
            className="cnav-btn" onClick={clearRoute}
            style={{
      ...
      <div style={{
        ...
        <button
          className="cnav-btn" onClick={reCenter}
          style={{
        ...
        {navigating && (
          <button
            ...
            style={{
              width: 44, height: 44, borderRadius: "50%",
              background: "rgba(14,14,14,0.96)", border: "1px solid #2a2a2a",
              color: "#ea4335", fontSize: 18, cursor: "pointer",
              display: "flex", alignItems: "center", justifyContent: "center",
              boxShadow: "0 4px 16px rgba(0,0,0,0.5)",
            }}
          >🧭</button>
        ...
        {(routeReady || navigating) && directions.length > 0 && !arrived && (
          <button
            className="cnav-btn" onClick={() => setShowStepsPanel((v) => !v)}
            style={{
              width: 44, height: 44, borderRadius: "50%",
              ...
              {routeInfo && (
                <div style={{ display: "flex", gap: 12, marginBottom: 14, justifyContent: "center" }}>
                  <div style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 28, fontWeight: 800, lineHeight: 1, color: navigating ? "#34a853" : "#4285F4" }}>
                      {routeInfo.time}
                    </div>
                  ...
                  {selectedDest && (
                    <>
                      ...
                      <div style={{ textAlign: "center", flex: 1 }}>
                        <div style={{
                          fontSize: 13, fontWeight: 600, color: "#ddd", lineHeight: 1,
                          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                        }}>{selectedDest.name}</div>
                ...
                {!navigating ? (
                  <>
                    {isPreviewMode ? (
                      ...
                      >☰ {showStepsPanel ? "Hide Directions" : "Show Directions"}</button>
                    ) : (
                      <button
                        className="cnav-btn" onClick={handleStartNavigation}
                        style={{
                          flex: 1, padding: "14px 0", background: "#34a853",
                          color: "#fff", border: "none", borderRadius: 14,
                    ...
                    <button
                      className="cnav-btn" onClick={clearRoute}
                      style={{
                        padding: "14px 18px", background: "#333",
                        color: "#fff", border: "none", borderRadius: 14,
                        cursor: "pointer", fontWeight: 700, fontSize: 16,
                      }}
                    >✕</button>
                  ...
                  <button
                    className="cnav-btn" onClick={clearRoute}
                    style={{
                      flex: 1, padding: "14px 0", background: "#d93025",
                      color: "#fff", border: "none", borderRadius: 14,
                      cursor: "pointer", fontWeight: 700, fontSize: 16,
                    }}
            ...
            <div style={{
              ...
              {routeInfo && (
                <div style={{ flex: 1, display: "flex", gap: 8, alignItems: "center" }}>
                  <span style={{ fontSize: 22, fontWeight: 800, color: navigating ? "#34a853" : "#4285F4" }}>
                    {routeInfo.time}
              ...
              {navigating ? (
                <button
                  className="cnav-btn" onClick={clearRoute}
                  style={{
                    padding: "10px 14px", background: "#d93025", color: "#fff",
                ...
                <button
                  className="cnav-btn" onClick={handleStartNavigation}
                  style={{
                    padding: "10px 14px", background: "#34a853", color: "#fff",
                    border: "none", borderRadius: 12, cursor: "pointer",
                    fontWeight: 700, fontSize: 13, flexShrink: 0,
                  }}
                >▶ Go</button>
            ...
            <div style={{ padding: "0 16px 16px", overflowY: "auto" }}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ position: "relative" }}>
                  <input
                    type="text" placeholder="Leave empty to use GPS…"
                    value={sourceQuery}
              ...
              <button
                className="cnav-btn" onClick={handleFindRoute} disabled={isSearching}
                style={{
              ...
              >{isSearching ? "Calculating…" : "Find Route"}</button>
            </div>
          </div>
        </>
      )}
