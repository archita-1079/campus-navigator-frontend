...
const normalizeNodeType = (type) => {
  ...
  if (["LIBRARY", "LAB", "ADMIN", "AUDITORIUM", "BUILDING"].includes(t)) return "BUILDING";
...
const buildDirections = (coords) => {
  ...
  const steps = [];
  let segStart = 0;
  let segBearing = getBearing(coords[0][1], coords[0][0], coords[1][1], coords[1][0]);
  let segDist = 0;

  for (let i = 0; i < coords.length - 1; i++) {
    segDist += getDistanceInMeters(coords[i][1], coords[i][0], coords[i+1][1], coords[i+1][0]);
    const isLast = i === coords.length - 2;
    if (isLast) {
      steps.push({ type: "arrive", bearing: segBearing, distance: Math.round(segDist), coordIndex: segStart });
      break;
    }
    const nextBearing = getBearing(coords[i+1][1], coords[i+1][0], coords[i+2][1], coords[i+2][0]);
    const turn = getTurnDirection(segBearing, nextBearing);
    if (turn !== "straight") {
      steps.push({ type: turn, bearing: segBearing, distance: Math.round(segDist), coordIndex: segStart });
      segStart = i + 1;
...
const buildArrowFeatures = (coordsArray, intervalM = 18) => {
  ...
  for (let i = 0; i < coordsArray.length - 1; i++) {
    ...
    while (offset <= segDist) {
      ...
      features.push({
        type: "Feature",
        geometry: { type: "Point", coordinates: [lng1 + (lng2 - lng1) * frac, lat1 + (lat2 - lat1) * frac] },
        properties: { bearing },
      });
      offset += intervalM;
    }
...
export default function CampusGraph() {
  ...
  // ── useGPS now auto-starts watchPosition internally ──────────────────────
  // Pass the new hook; it exports coords, error, isWatching
  // For backwards compat we keep the acquire() call below but it's no longer needed
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      // Accuracy circle
      map.addSource("accuracy-src", { type: "geojson", data: { type: "FeatureCollection", features: [] } });
      map.addLayer({
        id: "accuracy-fill", type: "circle", source: "accuracy-src",
        paint: {
      ...
      map.addLayer({
        id: "route-fill", type: "line", source: "route-src",
        layout: { "line-join": "round", "line-cap": "round" },
      ...
      map.addLayer({
        id: "route-arrows", type: "symbol", source: "arrows-src",
        layout: {
          "text-field": "▶",
          "text-size": 13,
  ...
  const renderMapData = useCallback((map, data) => {
    ...
    data.nodes
      .filter((n) => ["BUILDING","CANTEEN","HOSTEL","LIBRARY","LAB","ADMIN","AUDITORIUM"].includes(n.nodeType?.toUpperCase()))
      .forEach((node) => {
        ...
        Object.assign(lbl.style, { fontSize:"10px", color:"#fff", background:"rgba(0,0,0,0.6)", padding:"1px 5px", borderRadius:"3px", whiteSpace:"nowrap", maxWidth:"130px", overflow:"hidden", textOverflow:"ellipsis" });
        const ico = document.createElement("div");
        ico.innerHTML = cfg.icon;
        Object.assign(ico.style, { fontSize:"20px", color:cfg.color, lineHeight:1, filter:`drop-shadow(0 0 4px ${cfg.color})` });
  ...
  const flyToNode = (node) => {
    ...
    if (node.parentNodeId != null) {
      ...
      const lbl = document.createElement("div"); lbl.innerText = node.name;
      ...
      const ico = document.createElement("div"); ico.innerHTML = cfg.icon || "📍";
    ...
    try {
      ...
      if (map) {
        ...
        map.fitBounds(
          ...
          { padding: { top: 100, bottom: 140, left: 50, right: 320 }, pitch: 0, bearing: 0, duration: 1800 }
        );
      }
    } catch (e) {
  ...
  const handleStartNavigation = () => {
    setNavigating(true); setShowSteps(true);
    const map = mapInstance.current;
  ...
  const clearRoute = () => {
    ...
    setRouteCoords([]); setRouteReady(false); setNavigating(false);
    setDirections([]); setStepIdx(0); setRouteInfo(null);
    setShowSteps(false); setArrived(false);
    if (searchPinRef.current) { searchPinRef.current.remove(); searchPinRef.current = null; }
    const map = mapInstance.current;
    if (map) {
  ...
  const inputStyle = { width:"100%", padding:"8px 10px", marginTop:"5px", background:"#252525", color:"white", border:"1px solid #444", borderRadius:"7px", boxSizing:"border-box", fontSize:"13px", outline:"none" };
  const dropStyle  = { position:"absolute", top:"100%", left:0, right:0, background:"#252525", listStyle:"none", margin:0, padding:0, maxHeight:"150px", overflowY:"auto", border:"1px solid #555", borderRadius:"0 0 8px 8px", zIndex:30 };

  return (
    <div style={{ position: "relative" }}>

      {/* Search / Route panel */}
      <div style={{ position:"absolute", top:16, right:16, zIndex:20, background:"rgba(14,14,14,0.97)", color:"white", padding:"18px", borderRadius:"16px", width:"288px", boxShadow:"0 8px 32px rgba(0,0,0,0.65)", backdropFilter:"blur(12px)", border:"1px solid #2a2a2a" }}>
        <h3 style={{ margin:"0 0 12px", fontSize:"15px", fontWeight:700 }}>🗺️ Navigate Campus</h3>

        {/* GPS pill */}
        <div style={{ display:"flex", alignItems:"center", gap:"7px", marginBottom:"12px", padding:"6px 10px", background: coords ? "rgba(52,168,83,0.12)" : "rgba(234,67,53,0.1)", borderRadius:"8px", border:`1px solid ${coords ? "#34a85340" : "#ea433540"}`, fontSize:"11px", color: coords ? "#34a853" : "#ea4335" }}>
          <div style={{ width:"7px", height:"7px", borderRadius:"50%", background: coords ? "#34a853" : "#ea4335", boxShadow: coords ? "0 0 6px #34a853" : "none", animation: coords ? "gpsPulse 2s ease-out infinite" : "none", flexShrink:0 }} />
          {coords ? `GPS Active · ±${Math.round(coords.accuracy ?? 0)} m` : gpsError ? "GPS unavailable" : "Acquiring GPS…"}
        </div>

        {/* From */}
        <div style={{ position:"relative", marginBottom:"10px" }}>
          <label style={{ fontSize:"10px", color:"#666", textTransform:"uppercase", letterSpacing:"0.5px" }}>From</label>
          <input type="text" placeholder="Leave empty to use GPS…" value={sourceQuery}
            onChange={(e) => { handleSearch(e.target.value, true); setSelectedSource(null); }} style={inputStyle} />
          {sourceResults.length > 0 && !selectedSource && (
            <ul style={dropStyle}>
              {sourceResults.map((n) => { const name = getDisplayName(n); const meta = [n.parentNodeName, n.floor > 0 ? `Floor ${n.floor}` : null].filter(Boolean).join(" · "); return (
                <li key={n.id} style={{ padding:"8px 10px", cursor:"pointer", borderBottom:"1px solid #333" }}
                  ...
                  {meta && <div style={{ fontSize:"11px", color:"#777", marginTop:"2px" }}>{meta}</div>}
                </li>
              );})}
            </ul>
          )}
        ...
        <div style={{ position:"relative", marginBottom:"14px" }}>
          <label style={{ fontSize:"10px", color:"#666", textTransform:"uppercase", letterSpacing:"0.5px" }}>To</label>
          <input type="text" placeholder="Search destination…" value={destQuery}
            onChange={(e) => { handleSearch(e.target.value, false); setSelectedDest(null); }} style={inputStyle} />
          {destResults.length > 0 && !selectedDest && (
            <ul style={dropStyle}>
              {destResults.map((n) => { const name = getDisplayName(n); const meta = [n.parentNodeName, n.floor > 0 ? `Floor ${n.floor}` : null].filter(Boolean).join(" · "); return (
                <li key={n.id} style={{ padding:"8px 10px", cursor:"pointer", borderBottom:"1px solid #333" }}
                  onMouseEnter={(e) => e.currentTarget.style.background="#333"} onMouseLeave={(e) => e.currentTarget.style.background="transparent"}
        ...
        <div style={{ display:"flex", gap:"8px" }}>
          {!routeReady ? (
            <button className="cnav-btn" onClick={handleFindRoute} disabled={isSearching}
              style={{ flex:1, padding:"10px", background: isSearching ? "#2a4a8a" : "#4285F4", color:"#fff", border:"none", borderRadius:"9px", cursor: isSearching ? "not-allowed" : "pointer", fontWeight:700, fontSize:"14px" }}>
              {isSearching ? "Calculating…" : "Find Route"}
            </button>
          ) : !navigating ? (
            <>
              <button className="cnav-btn" onClick={handleStartNavigation}
                style={{ flex:1, padding:"10px", background:"#34a853", color:"#fff", border:"none", borderRadius:"9px", cursor:"pointer", fontWeight:700, fontSize:"14px" }}>
                ▶ Start Navigation
              </button>
              <button className="cnav-btn" onClick={clearRoute}
                style={{ padding:"10px 13px", background:"#d93025", color:"#fff", border:"none", borderRadius:"9px", cursor:"pointer", fontWeight:700 }}>✕</button>
            </>
          ) : (
            <button className="cnav-btn" onClick={clearRoute}
              style={{ flex:1, padding:"10px", background:"#d93025", color:"#fff", border:"none", borderRadius:"9px", cursor:"pointer", fontWeight:700, fontSize:"14px" }}>
              ✕ End Navigation
            </button>
          )}
        </div>
        ...
        {/* ETA */}
        {routeInfo && (
          <div style={{ marginTop:"12px", display:"flex", background:"#181818", borderRadius:"10px", overflow:"hidden", border:"1px solid #2a2a2a" }}>
            <div style={{ flex:1, textAlign:"center", padding:"10px 0" }}>
              <div style={{ fontSize:"22px", fontWeight:700, color: navigating ? "#34a853" : "#4285F4", lineHeight:1 }}>{routeInfo.time}</div>
              <div style={{ fontSize:"10px", color:"#555", marginTop:"2px" }}>min walk</div>
            </div>
            <div style={{ width:"1px", background:"#2a2a2a" }} />
            <div style={{ flex:1, textAlign:"center", padding:"10px 0" }}>
              <div style={{ fontSize:"18px", fontWeight:600, color:"#bbb", lineHeight:1 }}>{routeInfo.distance} m</div>
              <div style={{ fontSize:"10px", color:"#555", marginTop:"2px" }}>{navigating ? "remaining" : "total"}</div>
            </div>
          </div>
        )}
      </div>
      ...
      {navigating && dirInfo && !arrived && (
        <div style={{ position:"absolute", top:16, left:"50%", zIndex:25, animation:"dirSlide 0.3s ease", background:"rgba(10,10,10,0.97)", border:`2.5px solid ${dirInfo.color}`, borderRadius:"18px", padding:"13px 20px", display:"flex", alignItems:"center", gap:"13px", boxShadow:`0 6px 28px rgba(0,0,0,0.7)`, backdropFilter:"blur(14px)", minWidth:"230px" }}>
          <div style={{ width:"50px", height:"50px", borderRadius:"13px", background:`${dirInfo.color}18`, border:`2px solid ${dirInfo.color}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"26px", flexShrink:0 }}>
            {dirInfo.icon}
          </div>
          <div>
            <div style={{ fontSize:"16px", fontWeight:700, color:dirInfo.color }}>{dirInfo.label}</div>
            {activeStep?.distance > 0 && <div style={{ fontSize:"12px", color:"#888", marginTop:"3px" }}>in {activeStep.distance} m</div>}
          </div>
          <div style={{ marginLeft:"auto", fontSize:"10px", color:"#444", flexShrink:0 }}>{stepIdx+1}/{directions.length}</div>
        </div>
      )}

      {/* Arrived */}
      {arrived && (
        <div style={{ position:"absolute", top:"50%", left:"50%", zIndex:30, animation:"arrivalPop 0.4s ease", background:"rgba(10,10,10,0.97)", border:"2px solid #34a853", borderRadius:"20px", padding:"28px 40px", textAlign:"center", boxShadow:"0 12px 48px rgba(0,0,0,0.8)", transform:"translate(-50%,-50%)" }}>
          <div style={{ fontSize:"48px" }}>🏁</div>
          <div style={{ fontSize:"21px", fontWeight:700, color:"#34a853", marginTop:"8px" }}>You have arrived!</div>
          <div style={{ fontSize:"13px", color:"#777", marginTop:"4px" }}>{selectedDest?.name}</div>
          <button className="cnav-btn" onClick={clearRoute}
            style={{ marginTop:"16px", padding:"10px 28px", background:"#34a853", color:"#fff", border:"none", borderRadius:"10px", cursor:"pointer", fontWeight:700, fontSize:"14px" }}>
            Done
          </button>
        </div>
      )}
      ...
      {navigating && directions.length > 0 && !arrived && (
        <>
          <button className="cnav-btn" onClick={() => setShowSteps((v) => !v)}
            style={{ position:"absolute", bottom:110, left:16, zIndex:20, background:"rgba(14,14,14,0.95)", color:"#4285F4", border:"1px solid #4285F430", borderRadius:"10px", padding:"7px 13px", cursor:"pointer", fontSize:"12px", fontWeight:600, backdropFilter:"blur(8px)" }}>
            {showSteps ? "▲ Hide steps" : "▼ Show steps"}
          </button>
          {showSteps && (
            <div style={{ position:"absolute", bottom:150, left:16, zIndex:20, background:"rgba(12,12,12,0.97)", color:"white", borderRadius:"14px", padding:"12px", width:"226px", maxHeight:"280px", overflowY:"auto", boxShadow:"0 8px 28px rgba(0,0,0,0.65)", border:"1px solid #222", backdropFilter:"blur(10px)" }}>
              <div style={{ fontSize:"10px", color:"#444", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:"8px" }}>All Steps</div>
              {directions.map((step, i) => {
                const di = DIR[step.type] || DIR.straight;
                const isActive = i === stepIdx;
                return (
                  <div key={i} onClick={() => setStepIdx(i)} style={{ display:"flex", alignItems:"center", gap:"9px", padding:"7px 8px", borderRadius:"9px", cursor:"pointer", background: isActive ? `${di.color}18` : "transparent", border:`1px solid ${isActive ? di.color+"44" : "transparent"}`, marginBottom:"3px", transition:"all 0.15s" }}>
                    <div style={{ width:"28px", height:"28px", borderRadius:"7px", background:`${di.color}18`, border:`1px solid ${di.color}55`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"15px", flexShrink:0 }}>{di.icon}</div>
                    <div>
                      <div style={{ fontSize:"12px", color: isActive ? di.color : "#ccc", fontWeight: isActive ? 700 : 400 }}>{di.label}</div>
                      {step.distance > 0 && <div style={{ fontSize:"10px", color:"#555" }}>{step.distance} m</div>}
                    </div>
                  </div>
                );
              })}
