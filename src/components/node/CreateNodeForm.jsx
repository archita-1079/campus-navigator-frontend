...
const CreateNodeForm = ({ onCreated, toast, nodes }) => {
  ...
  // Fetch existing node data if in edit mode
  useEffect(() => {
    if (isEditMode) {
      const fetchNode = async () => {
        try {
          const res = await axios.get(`${API_ADMIN_BASE}/node/${id}`, {
            headers: {
          ...
          if (res.data.success) {
            ...
            setForm({
              name: data.name || "",
              nodeType: data.nodeType || "BUILDING",
  ...
  const fillGPS = () => {
    gps.acquire(
      (c) => {
        set("latitude", c.lat.toFixed(7));
        set("longitude", c.lng.toFixed(7));
  ...
  const submit = async () => {
    if (!form.nodeType || !form.latitude || !form.longitude) {
      toast.error("Node type, latitude & longitude are required");
      return;
    }
    setLoading(true);
    try {
      ...
      if (res.data.success) {
        toast.success(
          `Node ${isEditMode ? "updated" : "created"} — ID: ${res.data.data.id}`,
        );
        onCreated();
        if (isEditMode) {
          navigate("/list"); // Redirect back to list after editing
        } else {
          setForm({
            name: "",
            nodeType: "BUILDING",
  ...
  return (
    <div className="card">
      <div className="card-title">◈ {isEditMode ? "Edit" : "Create"} Node</div>
      <div className="form-grid">
        ...
        <div className="form-group full">
          ...
          <div className="gps-row">
            ...
            />
            <button
              className={`btn-gps${gps.acquiring ? " acquiring" : ""}`}
              onClick={fillGPS}
          ...
          <div className="hint">
            Click "Use GPS" to auto-fill from your device location
          </div>
        </div>
      ...
      <div className="btn-row">
        ...
        <button className="btn btn-primary" onClick={submit} disabled={loading}>
          {loading ? "Saving..." : isEditMode ? "Update Node" : "Create Node"}
        </button>
      </div>
    </div>
  );
